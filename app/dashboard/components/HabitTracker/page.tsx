...
const HabitTracker = () => {
  ...
  const stats = useMemo(() => {
    ...
    const totalStreak =
      habits.reduce((sum, habit) => sum + habit.current_streak, 0) || 0;
    const averageStreak =
    ...
    const dueToday = habits.filter((habit) => !habit.is_completed_today).length;
    const goalsAchieved =
  ...
  return (
    ...
    >
      ...
      <div className="hidden md:block flex-shrink-0 p-2">
        <StatsCards  darkMode={darkMode} habits = {habits} />
      </div>
          ...
          <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-700 rounded-lg shadow-sm hover:shadow-lg transition-shadow duration-300 p-5 mb-6">
            ...
            {stats.dueToday > 0 ? (
              <ul className="space-y-3">
                {habits
                  .filter((habit) => !habit.is_completed_today)
                  .map((habit) => (
                    <li
                      key={habit.id}
          ...
          <div className="p-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 p-5">
              ...
              <ul className="text-sm text-gray-600 dark:text-gray-300 space-y-2">
                ...
                <li>
                  <span className="font-medium">Habits Completed Today:</span>{" "}
                  {stats.completedToday}
                </li>
                <li>
                  <span className="font-medium">Goals Achieved:</span>{" "}
